...
const Sidebar: React.FC<SidebarProps> = ({ activeModule, onNavigate, onHome, onAdmin, isPremium, isPending, onUpgrade, userRole, isAdminView }) => {
  return (
    <aside className="w-20 md:w-64 bg-slate-900 border-r border-slate-800 min-h-screen flex flex-col transition-all duration-300 overflow-hidden">
      <div className="p-4 md:p-6 flex items-center gap-3 border-b border-slate-800 mb-4">
        <div className="bg-indigo-600 p-2 rounded-lg text-white">
          <GraduationCap size={24} />
        </div>
        <span className="font-bold text-xl hidden md:block text-slate-100">Portal</span>
      </div>
      ...
      <nav className="flex-1 px-3 space-y-1">
        ...
        <div className="pt-4 pb-2 px-3 text-[10px] font-bold text-slate-500 uppercase tracking-wider hidden md:block">
          Main Modules
        </div>
      ...
      <div className="p-4 border-t border-slate-800 mt-auto">
        {!isPremium ? (
          <div className="bg-gradient-to-br from-indigo-900/20 to-purple-900/20 p-4 rounded-xl border border-indigo-900/50 hidden md:block">
          ...
          <div className="bg-gradient-to-br from-amber-900/20 to-orange-900/20 p-4 rounded-xl border border-amber-900/50 hidden md:block">
